%========================================================================================
% Version: v1.0
% Course: EGB376 – Steel Structures
% Standard: AS 4100 (teaching-level use)
% Intended use: Open-book exams and revision
% Last updated: 2025
%========================================================================================
...
\usepackage{microtype}      % Better line breaking/kerning (helps prevent overfull lines)
...
\hypersetup{
    ...
    pdfauthor={Joseph McKenna}, % PDF metadata
}

%----------------------------------------------------------------------------------------
%   GLOBAL LINE-BREAK SAFETY (prevents content spilling into margins)
%----------------------------------------------------------------------------------------
% Allows TeX a bit more flexibility to avoid overfull hboxes (text running off the page)
...
\author{Prepared by: Joseph McKenna \\ \normalsize Instagram: @joseph.mckenna\textunderscore}
...
\textit{For usage instructions and disclaimers, see the README.md in the repository.}

\textit{This document is a study aid and workflow guide, not a replacement for the AS 4100 Standard.}
\vspace{1em}
\tableofcontents
...
%----------------------------------------------------------------------------------------
\section*{Advanced Load Analysis: Patterns \& Combinations}\label{sec:load_patterns}
\phantomsection
...
\begin{enumerate}
    \item \textbf{Start at the Master Decision Tree.} Answer the questions to identify the correct workflow for your problem.
    
    \textbf{Go to} \hyperref[sec:master]{Section \ref{sec:master}: \nameref{sec:master}}.
...
\vspace{0.5em}
\section*{Question Decoder (60-second triage)}\label{sec:decoder}
\begin{strategybox}
...
\begin{tcolorbox}[colback=gray!5, colframe=gray!50!black, fonttitle=\bfseries, title=Fast Index (quick jump---use the Master Decision Tree for full routing)]
\renewcommand{\arraystretch}{1.25}
\small
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{0.32\textwidth} >{\raggedright\arraybackslash}p{0.38\textwidth} >{\raggedright\arraybackslash}p{0.22\textwidth}@{}}
\textbf{If the prompt looks like...} & \textbf{Your first move is...} & \textbf{Go to} \\
\hline
Axial tension member / tie / plate in tension; net area, block shear, fracture & Compute $N^*$ then check member + connection (weakest link governs) & \hyperref[sec:tension]{Sec.\ \ref{sec:tension}: Tension} \\
Column / strut / compression member; buckling, slenderness, effective area & Compute $N^*$; check section capacity + member buckling & \hyperref[sec:compression]{Sec.\ \ref{sec:compression}: Compression} \\
Beam; bending + shear + web bearing; LTB / unbraced length & Compute $M^*, V^*$; check bending then shear then web local checks & \hyperref[sec:flexure]{Sec.\ \ref{sec:flexure}: Flexure} \\
Beam-column; combined $N^*$ and $M^*$; interaction check & Gather pure capacities first, then apply interaction checks & \hyperref[sec:beamcolumn]{Sec.\ \ref{sec:beamcolumn}: Beam-Column} \\
Eccentric welded bracket/plate; weld group under shear + moment & Resolve direct + torsional shear in weld group; check resultant vs capacity & \hyperref[sec:eccweld]{Sec.\ \ref{sec:eccweld}: Ecc.\ Welds} \\
Bolts in tension and shear; combined action on most critical bolt & Compute per-bolt actions; check shear, tension, then interaction & \hyperref[sec:bolts]{Sec.\ \ref{sec:bolts}: Bolts} \\
\end{tabularx}
\normalsize
\end{tcolorbox}
...
\begin{itemize}
    ...
    \item Material Properties: Steel Density $\rho = 7850$ kg/m$^3$, $E = 200,000$ MPa, $G_s = 80,000$ MPa.
\end{itemize}
\end{notebox}
\begin{notebox}
...
\begin{notebox}
All load combinations and pattern loading rules are defined in
\hyperref[sec:load_patterns]{Advanced Load Analysis: Patterns \& Combinations}.
Do not duplicate or guess combinations — refer to that section as the single source of truth.
\end{notebox}
...
\section{Master Decision Tree: START EVERY PROBLEM HERE}\label{sec:master}

\begin{strategybox}
This section is your \textbf{triage hub}. Use it to identify (1) the \textbf{limit state}, (2) whether the question is about a \textbf{member, connection, or full system}, and (3) the governing \textbf{actions} ($N^*, M^*, V^*$) so you can jump to the correct workflow fast.
\end{strategybox}

\begin{decisionbox}
\textbf{0. What are you being asked to do? (Verb Decoder)}
\begin{itemize}
    \item \textbf{IF} the question says \textbf{``check / verify / adequate? / PASS-FAIL''}:
        \begin{itemize}
            \item[\ding{226}] Do an \textbf{adequacy check}: compute $S^*$ then compare to $\phi R$.
        \end{itemize}
    \item \textbf{IF} the question says \textbf{``calculate capacity / determine maximum / find $\phi R$''}:
        \begin{itemize}
            \item[\ding{226}] Do a \textbf{capacity calculation}: compute governing $\phi R$ (often multiple checks, take minimum).
        \end{itemize}
    \item \textbf{IF} the question says \textbf{``select / design / size / number of bolts / weld size''}:
        \begin{itemize}
            \item[\ding{226}] Do a \textbf{sizing problem}: choose member/connection so capacity $\ge$ demand, then justify.
        \end{itemize}
\end{itemize}

\vspace{0.75em}\hrule\vspace{0.75em}

\textbf{1. Which limit state are you in? (Loads)}
\begin{itemize}
    \item \textbf{IF} the question mentions \textbf{yielding, buckling, fracture, ultimate, collapse}:
        \begin{itemize}
            \item[\ding{226}] \textbf{Strength (ULS)}: use factored combinations to find worst $N^*, M^*, V^*$.
            \item[\ding{226}] \textbf{Continuous beams / pattern loading:} If the structure is continuous, check \hyperref[sec:load_patterns]{\nameref{sec:load_patterns}} to identify the worst-case pattern.
        \end{itemize}
    \item \textbf{IF} the question mentions \textbf{deflection, drift, vibration, serviceability}:
        \begin{itemize}
            \item[\ding{226}] \textbf{Serviceability (SLS)}: use service combinations, then do \textbf{deflection} check (Section 5, Part E).
        \end{itemize}
\end{itemize}
...
\begin{notebox}
\textbf{STOP (Actions first):} Before choosing a workflow, compute the governing Design Action Effect ($S^*$) and extract the design actions ($N^*, M^*, V^*$). \textbf{No capacity checks until actions are pinned.}
\end{notebox}

\textbf{2. What is the object of design/check? (Member / Connection / System)}
\begin{itemize}
    \item \textbf{IF} the question is about a \textbf{member adequacy/capacity} (a beam/column/tie):
        \begin{itemize}
            \item[\ding{226}] Go to Step 3 (Primary action on the member).
        \end{itemize}
    \item \textbf{IF} the question is about a \textbf{connection detail only} (bolt group / weld group / bracket):
        \begin{itemize}
            \item[\ding{226}] Skip to Step 5 (Connection-only workflows).
        \end{itemize}
    \item \textbf{IF} it is a \textbf{tension system/splice} or a member + connection question:
        \begin{itemize}
            \item[\ding{226}] You must check \textbf{BOTH} member and connection (weakest link governs).
        \end{itemize}
\end{itemize}

\vspace{0.75em}\hrule\vspace{0.75em}

\textbf{3. What are the PRIMARY actions on the MEMBER?} \\
\textit{Choose based on the demand you must check (from $N^*, M^*, V^*$), not the member name in the question.}
...
\begin{itemize}
    \item \textbf{IF} the member is primarily resisting \textbf{Axial Tension} (being pulled apart by a force $N^*$), and the question asks for its design capacity...
        \begin{itemize}
            \item[\ding{226}] \textbf{Go to} \hyperref[sec:tension]{Section \ref{sec:tension}: \nameref{sec:tension}}
        \end{itemize}
    ...
    \item \textbf{IF} the member is primarily resisting \textbf{Axial Compression} (a column being squashed by a force $N^*$), and the question asks for its design capacity...
        \begin{itemize}
            \item[\ding{226}] \textbf{Go to} \hyperref[sec:compression]{Section \ref{sec:compression}: \nameref{sec:compression}}
        \end{itemize}
    ...
    \item \textbf{IF} the member is primarily resisting \textbf{Bending} (a beam supporting transverse loads, creating a moment $M^*$), and the question asks for its capacity or adequacy...
        \begin{itemize}
            \item[\ding{226}] \textbf{Go to} \hyperref[sec:flexure]{Section \ref{sec:flexure}: \nameref{sec:flexure}}
        \end{itemize}
    ...
    \item \textbf{IF} the member is resisting both \textbf{Axial Compression AND Bending} simultaneously (a beam-column with forces $N^*$ and $M^*$), and the question asks for its adequacy...
        \begin{itemize}
            \item[\ding{226}] \textbf{Go to} \hyperref[sec:beamcolumn]{Section \ref{sec:beamcolumn}: \nameref{sec:beamcolumn}}
        \end{itemize}

    \item \textbf{IF} the member has significant \textbf{shear and local web actions} (reactions/point loads) in addition to bending...
        \begin{itemize}
            \item[\ding{226}] In Section 5, do \textbf{ALL} of: bending + shear + web bearing/buckling (and stiffeners if needed).
        \end{itemize}
\end{itemize}

\vspace{0.75em}\hrule\vspace{0.75em}

\textbf{4. Special flags that change the workflow (don’t miss easy marks)}
\begin{itemize}
    \item \textbf{Slender / built-up / custom welded section:} you may need effective properties ($A_e, I_{eff}, Z_e$) -- do not blindly use tables.
    \item \textbf{Unbraced compression flange / LTB wording / restraint notes:} LTB may govern (Section 5, Step 2a--3).
    \item \textbf{Splice plates:} treat splice plates as members too; check their yielding/net fracture/block shear (Section 3 note).
    \item \textbf{Stability / uplift / overturning:} treat as a separate load direction case (often reduced $G$ with wind).
    \item \textbf{Eccentricity / offset load:} create a moment on the connection group ($T^* = V^* e$ or $M^* = N^* e$).
    \item \textbf{Holes in a tension flange/web (service holes):} check net section / block shear / local tearing where relevant.
    \item \textbf{Torsion / warping / fatigue / fire:} these can be assessed in steel design, but are not the focus of this checklist. If they appear, follow the question’s stated clause/method and write assumptions clearly.
\end{itemize}

\vspace{0.75em}\hrule\vspace{0.75em}

\textbf{5. Connection-only workflows (when the question is purely about the detail)}
...
\begin{itemize}
    \item \textbf{IF} the question shows a bracket or plate eccentrically \textbf{welded} to a column and asks for the adequacy of the weld...
        \begin{itemize}
            \item[\ding{226}] \textbf{Go to} \hyperref[sec:eccweld]{Section \ref{sec:eccweld}: \nameref{sec:eccweld}}
        \end{itemize}
    ...
    \item \textbf{IF} the question shows a \textbf{bolted} connection resisting pure Tension ($N_{tf}^*$) or a combination of Tension and Shear ($N_{tf}^*$ and $V_f^*$), and asks for the adequacy of the bolts...
        \begin{itemize}
            \item[\ding{226}] \textbf{Go to} \hyperref[sec:bolts]{Section \ref{sec:bolts}: \nameref{sec:bolts}}
        \end{itemize}

    \item \textbf{IF} the connection is \textbf{simple bolt shear/bearing} (no combined tension) inside a tension system problem...
        \begin{itemize}
            \item[\ding{226}] Stay in Section 3 Part B (connection checks as part of the system).
        \end{itemize}

    \item \textbf{Trap rule:} If the question includes a \textbf{member} \textit{and} a connection/detail sketch, assume you must check \textbf{both} unless it explicitly says ``connection only''.
\end{itemize}

\vspace{0.75em}\hrule\vspace{0.75em}

\begin{tcolorbox}[colback=green!6!white, colframe=green!60!black, fonttitle=\bfseries, title=Decision Output (write this before you turn the page), enhanced, breakable]
\begin{tabularx}{\textwidth}{@{}lX@{}}
\textbf{Verb type:} & \blank{10cm} \quad (Check / Capacity / Size) \\
\textbf{Limit state:} & \blank{10cm} \quad (ULS / SLS) \\
\textbf{Object:} & \blank{10cm} \quad (Member / Connection / System) \\
\textbf{Design actions to compute:} & \blank{10cm} \quad ($N^*, M^*, V^*$) \\
\textbf{Workflow selected:} & \blank{10cm} \quad (linked section title) \\
\end{tabularx}
\end{tcolorbox}
\end{decisionbox}
...
\section{Tension System Workflow ($N^* \le \phi N_{system}$)}\label{sec:tension}
...
\section{Compression Member Workflow ($N^* \le \phi N_c$)}\label{sec:compression}
...
\begin{tcolorbox}[colback=gray!5,colframe=gray!50,title=Phase 1: Setup \& Data Extraction]
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item Gross Area $A_g = \underline{\hspace{2cm}}$ mm$^2$ \quad \textit{(Net area $A_n$ is only relevant if holes exist.)}
...
\begin{itemize}
    ...
    \item \textbf{Slenderness Ratios ($l_e/r$):}
        ...
        \end{itemize}
    \item \textbf{Multiple Restraints:} If the column has intermediate braces (e.g., at 3 m, 6 m), it divides into segments. Calculate $l_e/r$ for \textbf{each segment} (using local $L$ and local $k_e$) and use the \textbf{maximum} value found.
...
\section{Flexural System (Beam) Workflow}\label{sec:flexure}
...
\begin{itemize}
    ...
    \item \textbf{Weld Design:} The weld must transfer load from stiffener to web. Design force $\approx N^* \times (A_{\text{stiff}} / A_{\text{total}})$.
    \item \textbf{Next:} After sizing stiffeners and welds, recheck Part C local web checks, then proceed to Part E and Part F.
\end{itemize}

\subsection{Part E: Serviceability (Deflection) Checks}
\begin{strategybox}
Serviceability checks use \textbf{service/unfactored} combinations. Deflection limits depend on the problem statement; if not given, use common limits (e.g., $L/250$ to $L/360$ for floors/roofs). \textbf{If not specified, state the assumed limit clearly before checking} (markers like to see the assumption written).
\end{strategybox}
\begin{itemize}
    ...
    \item \textbf{Step 3:} Check $\delta_{\max} \le \delta_{\text{allow}}$ (e.g., $\delta_{\text{allow}} = L/250$).
...
\begin{itemize}
    ...
    \item Serviceability (Deflection) Check: $\delta_{\max} = \underline{\hspace{2cm}}$ mm $\le \delta_{\text{allow}} = \underline{\hspace{2cm}}$ mm \quad (\textbf{PASS / FAIL})
\end{itemize}
...
\section{Combined Actions (Beam-Column) Workflow}\label{sec:beamcolumn}
...
\begin{notebox}
This section uses simplified interaction expressions commonly taught for exams.
If an assessment explicitly requires a clause-based solution,
you must use the exact AS 4100 equations instead.
\end{notebox}
...
\section{Advanced Connection: Eccentric Welds Workflow}\label{sec:eccweld}
...
\begin{itemize}
    ...
    \item Total Vertical Force: $v_{\text{total},y}^* = v_{v,y}^* + v_{t,y}^* = \underline{\hspace{2cm}} + \underline{\hspace{2cm}} = \underline{\hspace{2.5cm}}$ N/mm
    \item Total Horizontal Force: $v_{\text{total},x}^* = v_{v,x}^* + v_{t,x}^* = 0 + \underline{\hspace{2cm}} = \underline{\hspace{2.5cm}}$ N/mm
    \item \textbf{Resultant Force:} $v_{\text{res}}^* = \sqrt{(v_{\text{total}, x}^*)^2 + (v_{\text{total}, y}^*)^2} = \underline{\hspace{3cm}}$ N/mm
\end{itemize}
...
\begin{itemize}
    \item Maximum Resultant Force ($v_{\text{res}}^*$) = \underline{\hspace{3cm}} N/mm
    \item Design Weld Capacity ($\phi v_w$) = \underline{\hspace{3cm}} N/mm
\end{itemize}
\textbf{Check:} Is $v_{\text{res}}^* \le \phi v_w$? \quad \underline{YES / NO} \\
...
\begin{itemize}
    ...
    \item Is $v_{\text{res}}^*$ in N/mm? (Should be hundreds, not millions)
...
\begin{enumerate}
    ...
    \item Resultant: $v_{\text{res}}^* = \sqrt{v_x^2 + v_y^2} = $ \blank{3cm} N/mm
    \item Check: $v_{\text{res}}^* = $ \blank{2cm} $\le \phi v_w = $ \blank{2cm} N/mm $\Rightarrow$ \textbf{PASS / FAIL}
\end{enumerate}
\end{answerbox}
...
\section{Advanced Connection: Bolts in Combined Shear \& Tension}\label{sec:bolts}
...
\begin{itemize}
    ...
    \item \textbf{Determine reduction factor $k_r$:}
        \begin{itemize}
            \item If connection length $L_j < 300\text{ mm}$, $k_r = 1.0$.
            ...
            \item \textit{Example:} 1 row of 2 bolts at 60 mm spacing $\Rightarrow L_j = 60\text{ mm}$.
            \item \textit{Example:} 1 row of 10 bolts at 60 mm spacing $\Rightarrow L_j = 9 \times 60 = 540\text{ mm}$.
            \item If $L_j > 300\text{ mm}$, calculate $k_r = 1.075 - L_j/4000$.
        \end{itemize}
...
\appendix
%========================================================================================
%   APPENDIX A: DEFINITIONS OF SYMBOLS AND VARIABLES
%========================================================================================

\section{Definitions of Symbols and Variables}

\begin{strategybox}
This appendix defines all common symbols used throughout this document.
Symbols follow standard AS 4100 conventions unless otherwise stated.
Always check the governing clause if a question explicitly defines a symbol differently.
\end{strategybox}

\subsection*{Actions and Design Action Effects}
\begin{tabularx}{\textwidth}{@{}lX@{}}
$G$ & Permanent (dead) action \\
$Q$ & Imposed (live) action \\
$W_u$ & Ultimate wind action \\
$W_s$ & Serviceability wind action \\
$S^*$ & Design action effect (factored result of loads) \\
$N^*$ & Design axial force (positive = compression unless stated) \\
$M^*$ & Design bending moment \\
$V^*$ & Design shear force \\
$R^*$ & Design reaction or bearing force \\
$e$ & Eccentricity of load from centroidal axis \\
\end{tabularx}
...
$f_{yw}$ & Yield stress of web material \\
$f_{uf}$ & Ultimate tensile strength of bolt \\
$f_{uw}$ & Ultimate tensile strength of weld metal \\
$E$ & Young's modulus of steel (typically 200,000 MPa) \\
$G_s$ & Shear modulus of steel (typically 80,000 MPa) \\
\end{tabularx}

\subsection*{Geometric and Section Properties}
\begin{tabularx}{\textwidth}{@{}lX@{}}
$A_g$ & Gross cross-sectional area \\
$A_n$ & Net area after holes \\
$A_e$ & Effective area accounting for local buckling \\
$I_x, I_y$ & Second moments of area about principal axes \\
$I_{\text{eff}}$ & Effective second moment of area (local buckling) \\
$r_x, r_y$ & Radii of gyration \\
$S_x$ & Plastic section modulus \\
$Z_{ex}$ & Effective elastic section modulus \\
$t_w, t_f$ & Web and flange thicknesses \\
\end{tabularx}

\subsection*{Capacities and Reduction Factors}
\begin{tabularx}{\textwidth}{@{}lX@{}}
$\phi$ & Capacity reduction factor \\
$N_s$ & Nominal section compression capacity \\
$N_c$ & Nominal member compression capacity (buckling) \\
$M_s$ & Nominal section bending capacity \\
$M_b$ & Nominal member bending capacity (LTB) \\
$V_u$ & Nominal shear capacity \\
$\alpha_c$ & Compression buckling reduction factor \\
$\alpha_s$ & LTB reduction factor \\
$\alpha_m$ & Moment modification factor \\
$k_e$ & Effective length factor \\
$k_f$ & Form factor for compression sections \\
\end{tabularx}

\newpage
%========================================================================================
%   APPENDIX B: SHORT-ANSWER THEORY CRIB
%========================================================================================

\section{Short-Answer Theory Crib}

\begin{strategybox}
This section provides concise, exam-ready explanations for common short-answer questions. Use full sentences where marks are allocated for explanation.
\end{strategybox}

\subsection*{Buckling and Structural Behaviour}
\begin{itemize}
    \item \textbf{Local buckling:} Buckling of individual plate elements; may allow post-buckling strength redistribution.
    \item \textbf{Global buckling:} Instability of the whole member; typically brittle with little post-buckling strength.
    \item \textbf{LTB:} Lateral movement and twist of an unbraced compression flange in bending.
    \item \textbf{Post-buckling capacity:} Significant for local buckling, negligible for global buckling.
\end{itemize}

\subsection*{Design Concepts}
\begin{itemize}
    \item \textbf{Why use $\phi$ factors?} To account for material variability and modelling uncertainty.
    \item \textbf{Why reduce capacity instead of loads?} To ensure conservative design under uncertain strength.
    \item \textbf{Weakest link principle:} The system capacity is governed by the lowest member or connection capacity.
\end{itemize}

\subsection*{Connections}
\begin{itemize}
    \item \textbf{Purpose of $k_t$:} Accounts for non-uniform stress distribution in tension members.
    \item \textbf{Block shear:} Combined tension and shear failure along a defined path near a connection.
    \item \textbf{Prying:} Additional tension induced in bolts due to plate flexibility.
\end{itemize}

\newpage
%========================================================================================
%   APPENDIX C: HOW TO CONSTRUCT STRUCTURAL DIAGRAMS
%========================================================================================

\section{How to Construct Structural Diagrams (Step-by-Step)}

\begin{strategybox}
This appendix explains how to systematically construct structural diagrams
(BMD, SFD, and axial force diagrams) using words and logic rather than sketches.
Follow these steps before extracting design actions.
\end{strategybox}

\subsection*{Step 1: Identify the Structural System}
\begin{itemize}
    \item Identify supports (fixed, pinned, roller).
    \item Identify spans, cantilevers, and continuity.
    \item Determine whether the structure is determinate or indeterminate.
\end{itemize}

\subsection*{Step 2: Apply Loads}
\begin{itemize}
    \item Apply permanent loads ($G$) everywhere.
    \item Apply variable loads ($Q$, $S$, $W$) only where they increase the effect.
    \item For continuous members, apply pattern loading to maximise the response.
\end{itemize}

\subsection*{Step 3: Solve for Reactions}
\begin{itemize}
    \item Calculate support reactions before drawing any internal diagrams.
    \item Check equilibrium: $\sum F = 0$, $\sum M = 0$.
\end{itemize}

\subsection*{Step 4: Internal Force Progression}
\begin{itemize}
    \item Axial force is affected only by axial loads.
    \item Shear changes only where loads or reactions act.
    \item Moment is the integral of shear; maximum moment occurs where shear crosses zero.
\end{itemize}

\subsection*{Step 5: Identify Design Actions}
\begin{itemize}
    \item Locate maximum $N^*$, $M^*$, and $V^*$.
    \item Note whether bending is sagging or hogging.
    \item Identify the compression flange at critical sections.
\end{itemize}

\subsection*{Step 6: Sanity Checks}
\begin{itemize}
    \item Zero moment at pins and rollers.
    \item Maximum moment at fixed supports or midspan.
    \item Diagrams must satisfy equilibrium over the entire member.
\end{itemize}

\newpage
%========================================================================================
%   APPENDIX D: COMMON BEAM FORMULAS
%========================================================================================
